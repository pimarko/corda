...
the "propagated index" to indicate all errors have been dealt with. Subsequent ``DoRemainingWork`` events will thus do nothing. However, in
case some other error condition or external event adds another error to the flow, we would automatically propagate that too, we don't need
to write a special case for it.

Most of the state machine logic is therefore about the handling of ``DoRemainingWork``. Another example is resumptions due to an IO request
